...
tb2avgs <-
  ...
  group_by(pitch_name) %>%
  mutate(avgspeed = mean(release_speed, na.rm = TRUE)) %>%
  mutate(avgspin = mean(release_spin_rate, na.rm = TRUE)) %>%
  arrange(pitch_name)
tb2avgs
```
...
ju28avg <-
  tbju28%>%
  group_by(pitch_name) %>%
  summarise(avgspeed = mean(release_speed, na.rm = TRUE))
ju28avg
```
...
ju28avggr <-
  ju28avg%>%
  ggplot(aes(x = pitch_name, y = avgspeed)) +
